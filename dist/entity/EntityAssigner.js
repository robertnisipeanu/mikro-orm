...
class EntityAssigner {
    static assign(entity, data, onlyProperties = false) {
        ...
        Object.keys(data).forEach(prop => {
            ...
            if (props[prop] && [enums_1.ReferenceType.MANY_TO_ONE, enums_1.ReferenceType.ONE_TO_ONE].includes(props[prop].reference) && value && EntityAssigner.validateEM(em)) {
    ...
    static assignReference(entity, value, prop, em) {
        let valid = false;
        if (utils_1.Utils.isEntity(value) || value instanceof Reference_1.Reference) {
            entity[prop.name] = value;
            valid = true;
        }
        else if (utils_1.Utils.isPrimaryKey(value)) {

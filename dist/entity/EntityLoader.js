...
class EntityLoader {
    ...
    async populateMany(entityName, entities, field, where, orderBy, refresh) {
        ...
        const data = await this.findChildren(entities, prop, refresh, where[prop.name], innerOrderBy);
    ...
    async findChildren(entities, prop, refresh, where = {}, orderBy) {
        ...
        const ids = utils_1.Utils.unique(children.map(e => e.__primaryKey));
    ...
    async findChildrenFromPivotTable(filtered, prop, field, refresh, where, orderBy) {
        const map = await this.driver.loadFromPivotTable(prop, filtered.map(e => EntityHelper_1.wrap(e).__primaryKey), where, orderBy, this.em.getTransactionContext());
        const children = [];
        for (const entity of filtered) {
            const items = map[EntityHelper_1.wrap(entity).__primaryKey].map(item => this.em.merge(prop.type, item, refresh));
            entity[field].hydrate(items);
            children.push(...items);
        }
        return children;

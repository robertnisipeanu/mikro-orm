...
class QueryBuilder {
    ...
    prepareFields(fields, type = 'where') {
        ...
        Object.keys(this._populateMap).forEach(f => {
            ...
            if (this._joins[f].prop.reference !== entity_1.ReferenceType.ONE_TO_ONE) {
                utils_1.Utils.renameKey(this._cond, this._joins[f].inverseJoinColumn, `${this._joins[f].alias}.${this._joins[f].inverseJoinColumn}`);
            }
        });
        return ret;
    }
    ...
    autoJoinPivotTable(field) {
        ...
        utils_1.Utils.renameKey(this._cond, `${field}.${owner.name}`, `${pivotAlias}.${owner.fieldName}`);
        utils_1.Utils.renameKey(this._cond, `${field}.${inverse.name}`, `${pivotAlias}.${inverse.fieldName}`);
        this._populateMap[field] = this._joins[field].alias;
    }
}

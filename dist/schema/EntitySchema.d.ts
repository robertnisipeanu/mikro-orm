...
declare type Metadata<T, U> = Omit<Partial<EntityMetadata<T>>, 'name' | 'properties'> & ({
    ...
    properties?: {
        [K in keyof Omit<T, keyof U> & string]-?: Property<CollectionItem<NonNullable<T[K]>>>;
    };
};
export declare class EntitySchema<T extends AnyEntity<T> = AnyEntity, U extends AnyEntity<T> | undefined = undefined> {
    ...
    addIndex(options: Required<Omit<IndexOptions, 'name' | 'type'>> & {
        name?: string;
        type?: string;
    }): void;
    addUnique(options: Required<Omit<UniqueOptions, 'name'>> & {
        name?: string;
    }): void;

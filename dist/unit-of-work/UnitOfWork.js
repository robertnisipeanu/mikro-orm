...
class UnitOfWork {
    ...
    merge(entity, visited = [], mergeData = true) {
        ...
        this.cascade(entity, entity_1.Cascade.MERGE, visited);
    }
    ...
    async lock(entity, mode, version) {
        if (!this.getById(entity.constructor.name, entity_1.wrap(entity).__primaryKey)) {
    ...
    cascade(entity, type, visited, checkRemoveStack = false) {
        if (visited.includes(entity)) {
            return;
        }
        visited.push(entity);
        switch (type) {
            case entity_1.Cascade.PERSIST:
                this.persist(entity, visited, checkRemoveStack);
                break;
        ...
        for (const prop of Object.values(meta.properties).filter(prop => prop.reference !== entity_1.ReferenceType.SCALAR)) {
            this.cascadeReference(entity, prop, type, visited, checkRemoveStack);
        }
    }
    cascadeReference(entity, prop, type, visited, checkRemoveStack) {
        ...
        if ([entity_1.ReferenceType.MANY_TO_ONE, entity_1.ReferenceType.ONE_TO_ONE].includes(prop.reference) && reference) {
            return this.cascade(reference, type, visited, checkRemoveStack);
        }
    ...
    async lockPessimistic(entity, mode) {
        ...
        await qb.select('1').where({ [entity_1.wrap(entity).__meta.primaryKey]: entity_1.wrap(entity).__primaryKey }).setLockMode(mode).execute();
    }
